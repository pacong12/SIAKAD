...
<html lang="en">
  <head>
    ...
    <!-- Print orientation settings - multiple approaches for better browser support -->
    <style type="text/css">
      @page {
        size: landscape;
        margin: 1cm;
  ...
  <body>
    <div class="landscape-container">
      <div class="header">
        <h3>LAPORAN JADWAL MATA PELAJARAN</h3>
      ...
      @foreach ($jadwal->groupBy('kelas_id') as $kelasId => $jadwalPerKelas)
        @if($jadwalPerKelas[0]->kelas)
        <h5>Kelas: {{ $jadwalPerKelas[0]->kelas->nama_kelas }}</h5>
        @else
          <h5>Kelas: (Tidak Ditemukan)</h5>
        @endif
        ...
        @php
          ...
          // Mengumpulkan semua jam mulai yang unik untuk kelas ini
          foreach ($jadwalPerKelas as $j) {
        ...
        <table>
          ...
          <tbody>
            @foreach ($jamMulai as $index => $jam)
                  <tr>
                @if ($index === 0)
                  <td rowspan="{{ count($jamMulai) }}">
                    @if($jadwalPerKelas[0]->kelas)
                      {{ $jadwalPerKelas[0]->kelas->nama_kelas }}
                    @else
                      Kls ?
                    @endif
                  </td>
