...
<head>
  <meta charset="utf-8">
  <title>Laporan Prensensi Siswa</title>
  <style>
    body {
...
<body>
    ...
    <div style="float: left; width: 70%;">
      <table border="0" style="border: none;">
        <tr style="border: none;">
          ...
          <td style="border: none;">{{ count($absenPertanggal) }} data</td>
        </tr>
        <tr style="border: none;">
          <td style="border: none;"><strong>Periode</strong></td>
          <td style="border: none;">:</td>
          <td style="border: none;">{{ isset($tglawal) ? date('d/m/Y', strtotime($tglawal)) : '' }} s/d {{ isset($tglakhir) ? date('d/m/Y', strtotime($tglakhir)) : '' }}</td>
        </tr>
        <tr style="border: none;">
          <td style="border: none;"><strong>Status Hadir</strong></td>
          <td style="border: none;">:</td>
          <td style="border: none;">{{ $absenPertanggal->where('status', 'hadir')->count() }} siswa</td>
        </tr>
        <tr style="border: none;">
          <td style="border: none;"><strong>Status Sakit</strong></td>
          <td style="border: none;">:</td>
          <td style="border: none;">{{ $absenPertanggal->where('status', 'sakit')->count() }} siswa</td>
        </tr>
        <tr style="border: none;">
          <td style="border: none;"><strong>Status Izin</strong></td>
          <td style="border: none;">:</td>
          <td style="border: none;">{{ $absenPertanggal->where('status', 'izin')->count() }} siswa</td>
        </tr>
        <tr style="border: none;">
          <td style="border: none;"><strong>Status Alpa</strong></td>
          <td style="border: none;">:</td>
          <td style="border: none;">{{ $absenPertanggal->where('status', 'alpa')->count() }} siswa</td>
        </tr>
      </table>
    </div>
  ...
  <table>
    ...
    <tbody>
      @forelse ($absenPertanggal as $index => $p)
        <tr>
          <td class="text-center">{{ $index + 1 }}</td>
          <td>{{ \Carbon\Carbon::parse($p->tanggal)->format('d-m-Y') }}</td>
          <td>{{ $p->siswa->nisn ?? '-' }}</td>
          <td>{{ $p->siswa->nama ?? 'Data tidak tersedia' }}</td>
          <td class="text-center">{{ $p->kelas->nama_kelas ?? 'Data tidak tersedia' }}</td>
